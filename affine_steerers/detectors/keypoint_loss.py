...
class KeyPointLoss(nn.Module):
    ...
    def supervised_loss(self, outputs, batch, rot_A=0, rot_B=0, continuous_rot=False):
        keypoint_logits_A, keypoint_logits_B = outputs["keypoint_logits"].chunk(2)
        # rotate feature maps back for standard loss computation using upright GT
        if rot_A > 0:
            if continuous_rot:
                keypoint_logits_A = TTF.rotate(
                    keypoint_logits_A,
                    360. - np.rad2deg(rot_A),
                    interpolation=TTF.InterpolationMode.BILINEAR,
                )
            else:
                keypoint_logits_A = keypoint_logits_A.rot90(k=-rot_A, dims=[-2, -1])
        if rot_B > 0:
            if continuous_rot:
                keypoint_logits_B = TTF.rotate(
                    keypoint_logits_B,
                    360. - np.rad2deg(rot_B),
                    interpolation=TTF.InterpolationMode.BILINEAR,
                )
            else:
                keypoint_logits_B = keypoint_logits_B.rot90(k=-rot_B, dims=[-2, -1])
    ...
    def forward(self, outputs, batch, rot_A=0, rot_B=0, steerer=None, projector=None, continuous_rot=False):
        ...
        for output in outputs:
            losses = losses + self.supervised_loss(output,
                                                   batch,
                                                   rot_A=rot_A,
                                                   rot_B=rot_B,
