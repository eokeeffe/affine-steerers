...
from affine_steerers.utils import sample_keypoints, to_pixel_coords, to_normalized_coords, get_best_device


class affine_steerersDetector(nn.Module):
    ...
    def read_image(self, im_path, H = 784, W = 784, device=get_best_device()):
        pil_im = Image.open(im_path).resize((W, H)).convert('RGB')
