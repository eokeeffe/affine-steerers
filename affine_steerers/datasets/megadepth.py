...
from torchvision.transforms._functional_tensor import affine as tv_affine
from tqdm import tqdm

from affine_steerers.utils import get_depth_tuple_transform_ops, get_tuple_transform_ops
...
class MegadepthScene:
    def __init__(
        ...
        self.use_affine_aug = use_affine_aug
    ...
    def sample_affine(self):
        ...
        angle_2 = np.random.uniform(low=self.min_angle, high=self.max_angle)
        dilation_1 = np.exp(np.random.uniform(low=np.log(self.min_dilation), high=np.log(self.max_dilation)))
        dilation_2 = np.exp(np.random.uniform(low=np.log(self.min_dilation), high=np.log(self.max_dilation)))
        if np.random.uniform() < .5:
            dilation_1 *= -1
        return build_affine_double(angle_1, dilation_1, dilation_2, angle_2)

    def inv_affine_list(self, affine):
        inv = torch.linalg.inv(affine)
        return [inv[0, 0].item(), inv[0, 1].item(), 0., inv[1, 0].item(), inv[1, 1].item(), 0]

    def affine_around_center(self, affine):
        """ Input 2x2 affine map for image center (0, 0), output 3x3 map for image center ((wt-1)/2, (ht-1)/2). """
        T = torch.eye(3, device=affine.device)
        T[0, 2] = (self.wt - 1.) / 2
        T[1, 2] = (self.ht - 1.) / 2

        Tinv = torch.eye(3, device=affine.device)
        Tinv[0, 2] = -(self.wt - 1.) / 2
        Tinv[1, 2] = -(self.ht - 1.) / 2

        A = torch.eye(3, device=affine.device)
        A[:2, :2] = affine
        return T @ A @ Tinv

    def affine_aug(self, im_A, im_B, depth_A, depth_B, K_A, K_B, detections_A, detections_B):
        affine_A = self.sample_affine()
        affine_B = self.sample_affine()
        affine_AtoB = (affine_B @ torch.linalg.inv(affine_A)).to(im_A.device)

        affine_inv_list_A = self.inv_affine_list(affine_A)
        affine_inv_list_B = self.inv_affine_list(affine_B)

        im_A = tv_affine(im_A, affine_inv_list_A, interpolation="bilinear")
        im_B = tv_affine(im_B, affine_inv_list_B, interpolation="bilinear")

        depth_A = tv_affine(depth_A, affine_inv_list_A, interpolation="bilinear")
        depth_B = tv_affine(depth_B, affine_inv_list_B, interpolation="bilinear")

        coord_map_A = self.affine_around_center(affine_A).to(K_A.device)
        coord_map_B = self.affine_around_center(affine_B).to(K_B.device)

        K_A = coord_map_A @ K_A
        K_B = coord_map_B @ K_B

        if detections_A is not None:
            detections_A = detections_A @ coord_map_A[:2, :2].mT + coord_map_A[None, :2, 2]
        if detections_B is not None:
            detections_B = detections_B @ coord_map_B[:2, :2].mT + coord_map_B[None, :2, 2]

        return im_A, im_B, depth_A, depth_B, K_A, K_B, detections_A, detections_B, affine_AtoB
    ...
    def __getitem__(self, pair_idx):
        try:
            ...
            if affine_steerers.DEBUG_MODE:
                import matplotlib.pyplot as plt
                im = tensor_to_pil(depth_A[0], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_A[:30, 0], detections_A[:30, 1])
                plt.savefig(f"vis/depth_A_pre.jpg")
                plt.cla()
                im = tensor_to_pil(depth_B[0], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_B[:30, 0], detections_B[:30, 1])
                plt.savefig(f"vis/depth_B_pre.jpg")
                plt.cla()
                im = tensor_to_pil(im_A[:3], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_A[:30, 0], detections_A[:30, 1])
                plt.savefig(f"vis/im_A_pre.jpg")
                plt.cla()
                im = tensor_to_pil(im_B[:3], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_B[:30, 0], detections_B[:30, 1])
                plt.savefig(f"vis/im_B_pre.jpg")
                plt.cla()

            affine_AtoB = torch.eye(2)
            if self.use_affine_aug:
                im_A, im_B, depth_A, depth_B, K1, K2, detections_A, detections_B, affine_AtoB = self.affine_aug(
                    im_A, im_B, depth_A, depth_B, K1, K2, detections_A, detections_B,
                )
                affine_AtoB = affine_AtoB.float()
                    
            if self.use_horizontal_flip_aug:
                if self.use_affine_aug:
                    raise NotImplementedError(
                        "horizontal flips interfere with affine steering, flips are baked in to the affine augs"
                    )
            ...
            if affine_steerers.DEBUG_MODE:
                ...
                plt.imshow(im)
                plt.scatter(detections_A[:30, 0], detections_A[:30, 1])
                plt.savefig(f"vis/depth_A_post.jpg")
                plt.cla()
                im = tensor_to_pil(depth_B[0], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_B[:30, 0], detections_B[:30, 1])
                plt.savefig(f"vis/depth_B_post.jpg")
                plt.cla()
                im = tensor_to_pil(im_A[:3], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_A[:30, 0], detections_A[:30, 1])
                plt.savefig(f"vis/im_A_post.jpg")
                plt.cla()
                im = tensor_to_pil(im_B[:3], unnormalize=True)
                plt.imshow(im)
                plt.scatter(detections_B[:30, 0], detections_B[:30, 1])
                plt.savefig(f"vis/im_B_post.jpg")
                plt.cla()
            ...
            data_dict = {
                ...
                "im_A_path": im_A_ref,
                "im_B_path": im_B_ref,
                "affine_AtoB": affine_AtoB,
            }
            if affine_steerers.DEBUG_MODE:
                import pickle
                with open("vis/data.pkl", "wb") as handle:
