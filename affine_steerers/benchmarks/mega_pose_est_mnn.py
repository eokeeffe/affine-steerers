        ...
        with torch.no_grad():
            ...
            for scene_ind in range(len(self.scenes)):
                ...
                for pairind in (pbar := tqdm(pair_inds, desc = "Current AUC: ?")):
                    ...
                    detections_A = detector_model.detect_from_path(im_A_path, num_keypoints=self.num_keypoints)
                    keypoints_A, P_A = detections_A["keypoints"], detections_A["confidence"]
                    detections_B = detector_model.detect_from_path(im_B_path, num_keypoints=self.num_keypoints)
                    ...
                    if projector is not None:
                        description_A = projector(description_A)
                        description_B = projector(description_B)
                    ...
                    if hasattr(matcher_model, 'compensate_aspect_ratio') and matcher_model.compensate_aspect_ratio:
                        matches_A, matches_B, batch_ids = matcher_model.match(
                            keypoints_A, description_A,
                            keypoints_B, description_B,
                            P_A = P_A, P_B = P_B,
                            H_A=h1, W_A=w1, H_B=h2, W_B=w2,
                        )
                    else:
                        matches_A, matches_B, batch_ids = matcher_model.match(
                            keypoints_A, description_A,
