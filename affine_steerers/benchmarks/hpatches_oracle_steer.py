...
import torch
from affine_steerers.utils import pose_auc, to_homogeneous, from_homogeneous, local_affine_from_homography
import cv2



def normalize_H(Homog, H_A, W_A, H_B, W_B):
    T_A = torch.tensor([[[2 / W_A, 0, -1], [0, 2 / H_A, -1], [0, 0, 1]]], device=Homog.device, dtype=Homog.dtype)
    T_B = torch.tensor([[[2 / W_B, 0, -1], [0, 2 / H_B, -1], [0, 0, 1]]], device=Homog.device, dtype=Homog.dtype)
    H_n = T_B @ Homog @ torch.linalg.inv(T_A)
    return H_n
...
class HPatchesHomogMNN:
    ...
    def benchmark(self, detector, descriptor, steerer, matcher, model_name = None):
        n_matches = []
        homog_dists = []
        for seq_idx, seq_name in tqdm(
            ...
            descriptions_A = descriptor.describe_keypoints_from_path(im1_path, keypoints_A)["descriptions"]
            ...
            for im_idx in range(2, 7):
                ...
                H_n = normalize_H(torch.tensor(H).to(descriptions_A.device), h1, w1, h2, w2).float()
                ...
                with torch.inference_mode():
                    descriptions_A_steer = steerer(descriptions_A, affine_A_to_B)
                ...
                matches_A, matches_B, batch_ids = matcher.match(keypoints_A, descriptions_A_steer, 
