...
class MegadepthNLLBenchmark(nn.Module):
    ...
    def __init__(self, dataset, num_samples = 1000, batch_size = 8, matcher_model = DualSoftMaxMatcher(), device = "cuda") -> None:
        super().__init__()
    ...
    def compute_batch_metrics(self, detector, descriptor, batch, device = "cuda"):
        ...
        with torch.no_grad():
            ...
            inds = torch.nonzero((D_B == D_B.min(dim=-1, keepdim = True).values) 
                                 ...
                                 * (D_B < 0.01)
                                 * (D_A < 0.01))
        logP_A_B = self.matcher_model.log_softmax(descriptions_A, descriptions_B)
