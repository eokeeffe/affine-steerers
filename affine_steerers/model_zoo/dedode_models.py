...
from affine_steerers.detectors.dedode_detector import affine_steerersDetector
...
from affine_steerers.encoder import VGG19, VGG, VGG_DINOv2
...
def dedode_detector_B(device = get_best_device(), weights = None):
    ...
    model = affine_steerersDetector(encoder = encoder, decoder = decoder).to(device)
...
def dedode_detector_L(device = get_best_device(), weights = None, remove_borders = False):
    ...
    encoder = VGG19(pretrained = False, amp = amp, amp_dtype = amp_dtype)
    decoder = Decoder(conv_refiner)
    model = affine_steerersDetector(encoder = encoder, decoder = decoder, remove_borders = remove_borders).to(device)
    if weights is not None:
        model.load_state_dict(weights)
    return model


def dedode_descriptor_B_large(device = get_best_device(), weights = None):
    NUM_PROTOTYPES = 384 # == descriptor size
    residual = True
    hidden_blocks = 5
    amp_dtype = torch.float16#torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    amp = True
    ...
    model = affine_steerersDescriptor(encoder = encoder, decoder = decoder).to(device)    
    if weights is not None:
        model.load_state_dict(weights)
    return model
